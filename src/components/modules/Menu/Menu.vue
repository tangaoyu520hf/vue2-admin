  ...
  <imp-panel>
    <h3 class="box-title" slot="header" style="width: 100%;">
      <!--<el-button type="primary" icon="plus" @click="newAdd">新增</el-button>-->
       <el-button type="danger" :disabled='batchFlag' icon="delete" @click="batchDelete">删除</el-button>
    </h3>
    <el-row slot="body">
      <el-col :span="6">
        <el-tree v-if="menuTree"
                 ...
                 @node-click="handleNodeClick" @check-change="handleNodeCheckChange" clearable node-key="menuId" :props="defaultProps"></el-tree>
      </el-col>
      <el-col :span="18">
        <el-card class="box-card" style="margin-left: 15px">
          <div class="text item">
            <el-form :model="form" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
              ...
              <el-form-item prop="menuPid" label="父级" :label-width="formLabelWidth">
                <!--<el-input v-model="form.parentId" auto-complete="off"></el-input>-->
                <el-select-tree v-model="form.menuPid" :treeData="menuTreeOfApplication" :propNames="defaultProps" clearable
                                placeholder="请选择父级">
                </el-select-tree>
              </el-form-item>

              <el-form-item prop="menuName" label="名称" :label-width="formLabelWidth">
                <el-input v-model="form.menuName" auto-complete="off"></el-input>
              </el-form-item>
              <el-form-item prop="menuCode" label="编码" :label-width="formLabelWidth">
                <el-input v-model="form.menuCode" auto-complete="off"></el-input>
              </el-form-item>
              <el-form-item prop="menuUrl" label="菜单链接" :label-width="formLabelWidth">
                <el-input v-model="form.menuUrl" auto-complete="off"></el-input>
              </el-form-item>
              <el-form-item prop="menuIcon" label="图标" :label-width="formLabelWidth">
...
<script  type="text/babel">
  ...
  export default {
    ...
    computed: {
      batchFlag(){
        return !this.selectNodeKeys.length>0;
      }
    },
    data(){
      return {
    ...
    methods: {
          ...
          if (valid) {
            if (!this.form.menuId) {
              ...
              this.$http.post(api.SYS_MENU_UPDATE, this.form)
                .then(res => {
                  this.$message('操作成功');
                  this.updateTreeNode(this.menuTree, res.data.data,'menuId');
                })
            }
          }
        });
       ...
       */
      load(){
        this.$http.get(api.SYS_MENU_GETALLMENU)
          .then(res => {
    ...
    created(){
      //监控如果应用id发生了变化则 改版 需要刷选父级菜单
      this.$watch('form.applicationId', function (newVal, oldVal) {
